    ...
    <div className="min-h-screen w-full bg-[#0f1014] text-white">
      ...
      <nav className="flex justify-between items-center px-4 md:px-8 h-16 w-full bg-[#1a1a1a] border-b border-[#333] z-50 sticky top-0">
        ...
        <div className="hidden md:flex gap-4 items-center">
          {accessToken ? (
            <>
              <span className="text-gray-300 mr-2"><span className="text-[#FF1493] font-bold">{userInfo?.data?.name || "회원"}</span>님 반갑습니다.</span>
      ...
      <div className="flex">
        ...
        {/* 사이드바 */}
        <aside className={`fixed md:sticky md:top-16 top-0 left-0 h-screen w-64 bg-[#1a1a1a] border-r border-[#333] z-50 transform transition-transform ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 flex-shrink-0`}>
          <div className="p-4">
            ...
            <ul className="flex flex-col gap-2">
              ...
              {accessToken && (
                <li onClick={() => { navigate("/mypage"); setIsSidebarOpen(false); }} className="cursor-pointer hover:text-[#FF1493]">마이페이지</li>
              )}
            </ul>
          </div>
        </aside>
      ...
      {/* FAB 버튼 */}
      {accessToken && (
        <button onClick={() => setIsModalOpen(true)} className="fixed bottom-8 right-8 w-14 h-14 bg-[#FF1493] rounded-full flex justify-center items-center text-white text-3xl shadow-lg hover:scale-110 transition-transform z-50">+</button>
      )}
