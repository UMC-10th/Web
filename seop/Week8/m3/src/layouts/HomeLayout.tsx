  ...
  return (
    <div className="min-h-screen bg-[#1a1a2e] text-white">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 py-3 bg-[#1a1a2e] border-b border-gray-700">
        <div className="flex items-center gap-4">
          <button
            ...
            className="rounded p-1 text-gray-200 transition-colors hover:bg-white/10 hover:text-white"
          >
      ...
      <aside
        ...
        className={`fixed top-0 left-0 z-50 h-full w-60 border-r border-gray-700 bg-[#111122] pt-16 shadow-2xl transform transition-transform duration-300 ease-in-out ${
        isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
      }`}
      >
        <nav className="flex flex-col gap-2 px-4 py-6">
          <Link to="/" className="flex items-center gap-2 rounded px-3 py-2 text-gray-300 transition-colors hover:bg-white/10 hover:text-white" onClick={close}>
            🔍 찾기
          </Link>
          <Link to="/my" className="flex items-center gap-2 rounded px-3 py-2 text-gray-300 transition-colors hover:bg-white/10 hover:text-white" onClick={close}>
            👤 마이페이지
          </Link>
        </nav>
        {user && (
