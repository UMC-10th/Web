...
const fetchLps = async (sort: SortOrder, cursor?: number, search?: string): Promise<LpPage> => {
...
// ── 스켈레톤 ─────────────────────────────────────────
const SkeletonCard = () => (
  <div className="rounded-2xl animate-pulse" style={{ aspectRatio: "1/1", background: "#dbeafe" }} />
);
...
const CreateLpModal = ({ onClose }: CreateLpModalProps) => {
  ...
  const handleSubmit = async () => {
    ...
    createMutation.mutate({ title: title.trim(), content: content.trim(), thumbnail, published, tags });
  };
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold" style={{ color: "#1e3a8a" }}>새 LP 추가</h2>
          <button
            onClick={onClose}
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            style={{ background: published ? "#3b82f6" : "#cbd5e1" }}
            aria-label="공개 여부"
          >
            <span
              className="absolute top-0.5 w-4 h-4 rounded-full bg-white transition-transform"
              style={{ transform: published ? "translateX(21px)" : "translateX(2px)" }}
            />
          </button>
        ...
        <button
          ...
          style={{
            background: createMutation.isPending || !title.trim() ? "#94a3b8" : "linear-gradient(135deg,#1d4ed8,#3b82f6)",
...
export default function Home() {
  ...
  const scrollY = useThrottle(rawScrollY, 1000);

  useEffect(() => {
    const el = scrollContainerRef.current;
    if (!el) return;
    const handleScroll = () => setRawScrollY(el.scrollTop);
  ...
  const {
    ...
    queryKey: ["lps", sortOrder, trimmedQuery],
    // 빈 검색어일 때는 실행되지 않도록 설정 / enabled에다 적용하면 아에 쿼리 자체가 실행되지 않기 때문에 이렇게 구현함
    queryFn: ({ pageParam }) => fetchLps(sortOrder, pageParam as number | undefined, trimmedQuery || undefined),
      ...
      <main className="flex flex-col w-full h-full overflow-hidden">
        ...
        <div className="px-6 pt-4 pb-2 shrink-0">
          <span className="text-xs" style={{ color: "#94a3b8" }}>
            {isLoading ? "불러오는 중..." : `${lpList.length}개의 LP`}
          </span>
        </div>
        ...
        <div ref={scrollContainerRef} className="flex-1 overflow-y-auto px-6 pb-6">
          ...
          {!isLoading && !isError && (
            <>
              ...
              {isFetchingNextPage && (
                <div className="mt-4"><GridSkeleton count={5} /></div>
              )}
